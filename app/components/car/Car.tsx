...
const WheelJoint = ({
  ...
  useFrame(() => {
    if (joint.current && isDriving) {
      if (forwardPressed) {
        if (
          (leftPressed && wheelSide === WheelSide.FrontRight) ||
          (rightPressed && wheelSide === WheelSide.BackLeft)
        ) {
          joint.current.configureMotorVelocity(100, 10);
        } else if (
          (rightPressed && wheelSide === WheelSide.FrontLeft) ||
          (leftPressed && wheelSide === WheelSide.BackRight)
        ) {
          joint.current.configureMotorVelocity(100, 10);
